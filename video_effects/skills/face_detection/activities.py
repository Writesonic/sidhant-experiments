...
from video_effects.skills.face_detection.schemas import (
    DetectFacesRequest,
    BuildRemotionContextRequest,
)


@register_activity(name="vfx_detect_faces", description="Run full-video face detection")
def vfx_detect_faces(input_data: dict) -> dict:
    request = DetectFacesRequest(**input_data)
...
@register_activity(name="vfx_build_remotion_context", description="Build spatial context for Remotion MG planning")
def vfx_build_remotion_context(input_data: dict) -> dict:
    request = BuildRemotionContextRequest(**input_data)
    from video_effects.activities.remotion import build_remotion_context as _legacy_build
    return _legacy_build(request.model_dump())
