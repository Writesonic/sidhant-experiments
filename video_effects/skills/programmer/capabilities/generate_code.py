...
from video_effects.skills.programmer.schemas import ProgrammerGenerateCodeRequest, ProgrammerGenerateCodeResponse
...
class ProgrammerGenerateCodeCapability(BaseCapability[ProgrammerGenerateCodeRequest, ProgrammerGenerateCodeResponse]):
    async def execute(self, request):
        ...
        raw_code = call_text(system_prompt=system_prompt, user_message=user_message, model=settings.PROGRAMMER_LLM_MODEL)
        tsx_code = strip_markdown_fences(raw_code)
        props = dict(spec.get("data", {}))
        return ProgrammerGenerateCodeResponse(component_id=component_id, tsx_code=tsx_code, export_name=export_name, props=props)
